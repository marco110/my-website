...
node() {
    ...
    stage('ssh server & pull image'){
        try {
            ...
            // 启动
            sshCommand remote: sshServer, command: "docker run -u root --name ${dockerName} -v /home/marco/nginx/cert:/etc/nginx -p 80:80 -p 443:443 -d ${registry}/${aliyunNamespace}:${dockerTag}"
            // 只保留3个最新的镜像
